...
def google_login():
    ...
    # Get parameter to force account selection
    force_select = request.args.get('select_account', 'false')
    
    client_kwargs = {'scope': 'openid email profile'}
    
    # If user wants to switch accounts, force account selection
...
def google_authorize():
    """Handle Google OAuth callback - Only for invited operators"""
    try:
        ...
        # If user doesn't exist - not invited
        if not user:
            flash('Access Denied: You are not authorized to use Google Sign-In. Please use email/password login or contact administrator for an invitation.', 'error')
            return redirect(url_for('login'))
        
        # Check if user is trying to login as operator without being invited
        if user.role == 'operator' and not user.is_active:
            flash('Your operator account is pending activation. Please use the invitation link sent to your email.', 'error')
            return redirect(url_for('login'))
        
        # For regular commuters - prevent Google login unless they have a password set
        if user.role == 'commuter':
            # Commuters cannot use Google login - only email/password
            flash('Google Sign-In is only available for invited operators. Please use your email and password to login.', 'error')
            log_activity(user.id, user.role, user.username, 'login_failed', 'Google login attempt by commuter')
            return redirect(url_for('login'))
        
        # For operators - they must be active and have been invited
        if user.role == 'operator':
            if not user.is_active:
                flash('Your operator account is not yet activated. Please check your email for the invitation link.', 'error')
                return redirect(url_for('login'))
            ...
            # Successful operator login via Google
            ...
            # Log the activity
            log_activity(user.id, user.role, user.username, 'login', 'Operator logged in via Google OAuth')
            
            flash(f'Welcome back, {name}!', 'success')
            return redirect(url_for('operator_dashboard'))
        
        # For admin - allow Google login if they have Google ID
        if user.role == 'admin':
            if not user.google_id:
                user.google_id = user_info.get('sub')
            ...
            log_activity(user.id, user.role, user.username, 'login', 'Admin logged in via Google OAuth')
            
            flash(f'Welcome back, {name}!', 'success')
            return redirect(url_for('admin_dashboard'))
        
        # Fallback - should not reach here
        flash('Invalid account type. Please contact administrator.', 'error')
        return redirect(url_for('login'))
...
def get_schedule(station_name):
    ...
    try:
        ...
        if "error" in schedule:
            return jsonify({"error": schedule["error"], "status": "closed"})
...
def get_next_trains(station_name):
    ...
    try:
        ...
        return jsonify({
            "northbound": {
                "minutes": trains["northbound"]["minutes"],
                "origin": trains["northbound"]["from_station"],  # ← RENAME
                "status": trains["northbound"]["status"]
            },
            "southbound": {
                "minutes": trains["southbound"]["minutes"],
                "origin": trains["southbound"]["from_station"],  # ← RENAME
                "status": trains["southbound"]["status"]
            },
            "headway": trains["headway"],
            "is_operating": True,
            "is_closed": False  # ← ADD THIS
        })
    except Exception as e:
        print(f"❌ Error getting next trains: {e}")
        return jsonify({
            "is_closed": True,
            "northbound": {"minutes": 8, "origin": "Taft", "status": "estimated"},
            "southbound": {"minutes": 10, "origin": "North Ave", "status": "estimated"},
            "headway": 6
        })
...
def login():
    ...
    if request.method == 'POST':
        ...
        if admin_email and admin_password and email == admin_email and password == admin_password:
            ...
            # Log admin login
            log_activity(0, 'admin', email, 'login', f'Admin logged in from IP: {request.remote_addr}')
            return redirect(url_for('admin_dashboard'))
        ...
        if user is None:
            ...
            log_activity(None, 'unknown', email, 'login_failed', f'Account not found from IP: {request.remote_addr}')
        elif not user.is_active:
            error = "Account deactivated."
            log_activity(user.id, user.role, user.username, 'login_failed', f'Account deactivated from IP: {request.remote_addr}')
        elif user.google_id and not user.has_password():
            error = "This account uses Google Sign-In. Please click 'Continue with Google'."
            log_activity(user.id, user.role, user.username, 'login_failed', 'Attempted password login on Google account')
            ...
            log_activity(user.id, user.role, user.username, 'login', 
                        f'Successful login from IP: {request.remote_addr}')
            ...
            log_activity(user.id, user.role, user.username, 'login_failed', f'Incorrect password from IP: {request.remote_addr}')
...
def admin_generate_invite():
    """Generate operator invite link for ANY station"""
    try:
        data = request.json
        name = data.get('name')
        email = data.get('email')
        station = data.get('station')  # Can be ANY station name
        access_level_type = data.get('access_level', 'standard')  # 'full', 'standard', 'limited'
        
        print(f"📝 Creating operator: {name}, {email}")
        print(f"   Assigned Station: {station}")
        print(f"   Access Level: {access_level_type}")
        ...
        # Define all stations for reference
        STATIONS = ["North Ave", "Quezon Ave", "Kamuning", "Cubao", "Santolan", 
                    "Ortigas", "Shaw Blvd", "Boni Ave", "Guadalupe", "Buendia", 
                    "Ayala Ave", "Magallanes", "Taft"]
        ...
        # Special handling for "All Stations" option
        ...
        print(f"   Final: Access Level={db_access_level}, Stations={assigned_stations}")
        ...
        log_activity(admin_id, 'admin', admin_email, 'create_operator', 
                    f'Created operator: {email} | Access: {db_access_level} | Stations: {assigned_stations}')
        ...
        return jsonify({
            'success': True,
            'link': invite_link,
            'message': f'Operator created for {", ".join(assigned_stations)}'
        })
