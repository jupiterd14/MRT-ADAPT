...
def get_p95_percentile_wrapper(station_name, direction):
    """Wrapper to avoid circular imports"""
    from routes.api_predict import get_p95_percentile
    return get_p95_percentile(station_name, direction)
...
# ========== FIXED: USE P95 FOR CONGESTION CALCULATION ==========
def _get_congestion_from_prediction(pred_scaled, target_scaler, station_name, direction):
    ...
    # ========== USE P95 INSTEAD OF CAPACITY ==========
    p95 = get_p95_for_station(station_name, direction)
...
def live_map_directions():
    """Get congestion data for both directions - uses P95-based scaling (MATCHES api_predict)"""
    try:
        from flask import current_app
        ...
        def get_direct_prediction(station_name, direction):
            """Get prediction using App Config model references with P95-based scaling."""
            from flask import current_app
        ...
        if current_time < OPERATING_START or current_time >= OPERATING_END:
            ...
            for station in stations_list:
                ...
                if north_override_key in active_overrides:
                    ...
                    north_passengers = int((north_congestion / 100) * p95_north)
                else:
                ...
                if south_override_key in active_overrides:
                    ...
                    south_passengers = int((south_congestion / 100) * p95_south)
                else:
...
def debug_raw_prediction():
    """Get raw model output for debugging with P95-based scaling"""
    from services import get_feature_sequence_for_station
    ...
    try:
        ...
        for station in stations_list:
            ...
            northbound[station] = {
                ...
                "wait_time": north_wait,
                "status": north_status,
                "ridership": int((north_pred / 100) * p95_north),
                "p95": round(p95_north, 0)
            }
            southbound[station] = {
                "congestion": round(south_pred, 1),
                "wait_time": south_wait,
                "status": south_status,
                "ridership": int((south_pred / 100) * p95_south),
                "p95": round(p95_south, 0)
            }
