...
# ========== CORRECTED PRE-FILTER AND PRE-PROCESS ==========
print("\n" + "="*60)
...
for station_idx, station in enumerate(STATIONS):
    ...
    # ========== FIX: Separate flows for each direction ==========
    for direction in ['Northbound', 'Southbound']:
        
        # CORRECT FILTERING LOGIC:
        if direction == 'Northbound':
            # Northbound passengers EXIT at this station (coming from south)
            station_df = df[df['StationExit'] == station_num].copy()
            flow_type = "EXITS"
        else:  # Southbound
            # Southbound passengers ENTER at this station (going to Taft)
        ...
        # Aggregate by hour only (direction already filtered)
        ...
        # Calculate max from CORRECT passenger counts (only this direction's flow)
        max_hourly = dir_df['TotalPassenger'].quantile(0.99)
        if max_hourly == 0:
            max_hourly = 1
        
        # Calculate congestion using CORRECT max
        dir_df['congestion'] = (dir_df['TotalPassenger'] / max_hourly * 100).clip(0, 100)
        
                # Prepare features
                # ========== FIXED: NO DATA LEAKAGE ==========
        # Split temporally BEFORE any scaling
        n = len(dir_df)
        train_size = int(n * 0.8)

        # Split into train and validation sets (preserve time order)
        ...
        print(f"  [{direction}] Train sequences: {len(X_train):,} | Val sequences: {len(X_val):,}")
        ...
        print(f"    Max passengers (99th percentile): {max_hourly:.0f} {flow_type}/hour")  # ← FIXED INDENTATION
