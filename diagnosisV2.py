"""
BATCH TESTING SCRIPT - LSTM Congestion Forecasting
Runs multiple tests and appends all results to the same CSV files
"""
...
def run_test(STATION_NAME, DIRECTION, target_time):
    ...
    # Filter and aggregate data for this station/direction
    station_num = STATION_NUMBERS[STATION_NAME]
    print(f"\n Testing: {STATION_NAME} (Station #{station_num}) {DIRECTION} at {target_time}")
    ...
    if actual_row.empty:
        print(f"❌ No data for {target_time}")
    ...
    # Load model and scalers for this station/direction
    try:
    ...
    # Print result
    print(f"   Predicted: {pred_congestion:.1f}% | Actual: {actual_congestion:.1f}% | Error: {absolute_error:.1f} pts | {verdict}")
    
    # Save to results file (one per station-direction pair)
...
print("="*60)
print(" BATCH TESTING - LSTM Congestion Forecasting")
print("="*60)
...
# Load or create max passengers file
...
# batch_tests = []
# stations_to_test = ["Magallanes", "Ayala Ave", "Cubao"]
# dates_to_test = ["2025-01-06", "2025-01-07", "2025-01-08"]
# times_to_test = ["08:00:00", "12:00:00", "17:00:00"]
# 
# for station in stations_to_test:
#     for direction in ["Northbound", "Southbound"]:
#         for date in dates_to_test:
#             for time in times_to_test:
#                 batch_tests.append((station, direction, f"{date} {time}"))

# Option 3: Test all stations at specific times (uncomment to use)
# batch_tests = []
# all_stations = list(STATION_NUMBERS.keys())
# test_datetime = "2025-01-07 12:00:00"
# 
# for station in all_stations:
#     for direction in ["Northbound", "Southbound"]:
#         batch_tests.append((station, direction, test_datetime))

# ========== RUN BATCH TESTS ==========
print("\n" + "="*60)
print(f" BATCH TESTING: {len(batch_tests)} tests to run")
print("="*60)

successful_tests = 0
failed_tests = 0

for i, (station, direction, time_str) in enumerate(batch_tests, 1):
    print(f"\n[{i}/{len(batch_tests)}] ", end="")
    target_time = pd.to_datetime(time_str)
    
    if run_test(station, direction, target_time):
        successful_tests += 1
    else:
        failed_tests += 1
...
for file in os.listdir(RESULTS_FOLDER):
    if file.endswith('.csv'):
        ...
        print(f"   - {file}: {line_count} test results")
print("="*60)